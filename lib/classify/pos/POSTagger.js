...
POSTagger.prototype.tag = function(words) {
  ...
  for (var i = 0, size = words.length; i < size; i++) {
    ...
    if (!ss && words[i].length == 1) ret[i] = words[i] + "^"
    if (!ss) ret[i] = "NN"
    else ret[i] = ss[0]
  }
  ...
  for (var i = 0; i < words.length; i++) {
    ...
    if (startsWith(word, "N")) {
      if (words[i].indexOf(".") > -1) {
        ...
        if (/[a-zA-Z]{2}/.test(words[i])) ret[i] = "URL"
        else ret[i] = "CD"
      }
      // Attempt to convert into a number
      if (parseFloat(words[i])) ret[i] = "CD"
    }
    // rule 3: convert a noun to a past participle if words[i] ends with "ed"
    if (startsWith(ret[i], "N") && endsWith(words[i], "ed")) ret[i] = "VBN"
    // rule 4: convert any type to adverb if it ends in "ly";
    if (endsWith(words[i], "ly")) ret[i] = "RB"
    // rule 5: convert a common noun (NN or NNS) to a adjective if it ends with "al"
    if (startsWith(ret[i], "NN") && endsWith(word, "al")) (ret[i] = i), "JJ"
    // rule 6: convert a noun to a verb if the preceding work is "would"
    if (
      i > 0 &&
      startsWith(ret[i], "NN") &&
      words[i - 1].toLowerCase() == "would"
    )
      ret[i] = "VB"
    // rule 7: if a word has been categorized as a common noun and it ends with "s",
    //         then set its type to plural common noun (NNS)
    if (ret[i] == "NN" && endsWith(words[i], "s")) ret[i] = "NNS"
    // rule 8: convert a common noun to a present participle verb (i.e., a gerund)
    if (startsWith(ret[i], "NN") && endsWith(words[i], "ing")) ret[i] = "VBG"
  }
