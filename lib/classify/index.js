...
function getBetween(lex, type1, type2, form) {
  ...
  filter1 =
    tagged.filter(function(i) {
      return type1.indexOf(i[1]) !== -1
    }) || []
  filter2 =
    tagged.filter(function(i) {
      return type2.indexOf(i[1]) !== -1
    }) || []
  ...
  return start || end
    ? lex.slice(lex.indexOf(start) + 1, lex.indexOf(end) + 1)
    : []
}
...
module.exports = function classify(speech, debug) {
  ...
  if (
    getType(words[0]) === "VBZ" &&
    words.indexOf(action) === words.length - 2
  ) {
    subject = words
      .slice(1, -2)
      .join(" ")
    ...
    owner === "I" &&
    getType(words[words.indexOf(owner) + 1]) === "VBP"
  ) {
    answer = words.slice(0, words.indexOf(owner))

    subject = getBetween(answer, ["JJ"], ["VBP"])
      .slice(0, -1)
      .join(" ")
  ...
  subject =
    subject && subject.replace(/\s/gi, "").match(dirEx)
      ? subject.replace(/\s/gi, "")
      : subject
  owner =
    owner && owner.replace(/\s/gi, "").match(dirEx)
      ? owner.replace(/\s/gi, "")
      : owner
